...
def init_es_index(
    service: SearchIndexService = Depends(get_search_index_service),
) -> SearchIndexInitResponse:
    """
    初始化 ES BM25 index。
    """
...
def run_es_index_tasks(
    ...
    return service.run_tasks(limit=limit)
...
async def test_vector_search(
    ...
    return await service.search_vector(
        query=q,
        top_k=top_k,
    )
...
async def test_hybrid_search(
    ...
    return await service.search_hybrid(
        query=q,
        top_k=top_k,
    )
